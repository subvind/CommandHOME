<script lang="ts">
  ...
  let filterText: any;
  let value: any = organization.splashPhoto?.filename || '';
  let loading = true;
...
<div class="detail">
  <div class="row">
    <div class="col s12 m6">
      <br />
      {#if loading === false}
        <div>Select a splashPhoto: {filterText}</div>
      ...
      {#if organization.splashPhoto}
        <br />
        <img src={`https://s3.us-east-2.amazonaws.com/${organization.orgname}.${organization.splashPhoto.bucket.name}/${organization.splashPhoto.filename}`} alt="">
      {/if}
    </div>
    <div class="col s12 m6">
      <p>Use a photo that best describes the organization in detail. This photo will be show in the splashpage landing or homepage next to the organization's logo on the frontned.</p>
